...
// Файлы словарей плохих слов храняться на локальном сервере по адресу RU-CMSS13/config/chatfilter/

GLOBAL_LIST_INIT(bad_words, file2list("config/chatfilter/bad_words.txt"))
GLOBAL_LIST_INIT(exc_full, file2list("config/chatfilter/exc_full.txt"))
GLOBAL_LIST_INIT(ic_autoemote, params2list(file2text("config/chatfilter/emote.txt")))	// файл в текст, текст в лист
GLOBAL_LIST_INIT(ic_autocorrect, params2list(file2text("config/chatfilter/correct.txt")))
...
/client/proc/manage_chatfilter()
	...
	var/list/listoflists = list(
		"Словарь плохих слов" = list(GLOB.bad_words, "config/chatfilter/bad_words.txt"),
		"Словарь исключений" = list(GLOB.exc_full, "config/chatfilter/exc_full.txt"),
		"Словарь автоэмоутов" = list(GLOB.ic_autoemote, "config/chatfilter/emote.txt"),
		"Словарь автозамены" = list(GLOB.ic_autocorrect, "config/chatfilter/correct.txt")
		)
	...
	if(selected == "Словарь автоэмоутов" || selected == "Словарь автозамены")
		...
		GLOB.ic_autoemote = params2list(file2text("config/chatfilter/emote.txt"))
		GLOB.ic_autocorrect = params2list(file2text("config/chatfilter/correct.txt"))
	else
