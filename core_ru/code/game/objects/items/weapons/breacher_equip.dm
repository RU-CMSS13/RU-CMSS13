...
/obj/item/weapon/twohanded/breacher/attack(mob/target, mob/user)
...
/obj/item/weapon/twohanded/breacher/pickup(mob/user)
	RegisterSignal(user, COMSIG_HUMAN_POST_MOVE_DELAY, PROC_REF(handle_movedelay))
	. = ..()

/obj/item/weapon/twohanded/breacher/proc/handle_movedelay(mob/user, list/movedata)
	SIGNAL_HANDLER
	movedata["move_delay"] += speed_penalty

/obj/item/weapon/twohanded/breacher/dropped(mob/user, silent)
	. = ..()
...
/obj/item/weapon/twohanded/breacher/retrieve_to_slot(mob/living/carbon/human/user, retrieval_slot)
	if(!isturf(loc) || !user)
		return FALSE
