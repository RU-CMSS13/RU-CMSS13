...
/datum/battlepass_challenge/New(client/owner)
	...
	if(!completion_xp)
		...
		if(SSbattlepass.actual_challenges[code_name] && length(SSbattlepass.actual_challenges[code_name]["xp_array"]))
			completion_xp = rand(SSbattlepass.actual_challenges[code_name]["xp_array"][1], SSbattlepass.actual_challenges[code_name]["xp_array"][2])
