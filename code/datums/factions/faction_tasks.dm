...
/datum/faction_task/proc/announce_faction()
	marine_announcement(state & OBJECTIVE_COMPLETE ? announce_desc["complete"] : announce_desc["failed"], name, null, faction_owner)
...
/obj/structure/prop/sector_center/process()
	...
	for(var/atom in candidates)
		...
		if(carbon.faction)
			if(!length(sorted_in_range[carbon.faction]))
				sorted_in_range[carbon.faction] = list()
			total_ammount++
			sorted_in_range[carbon.faction] += carbon

	var/potential_faction
	...
	for(var/faction_to_get in sorted_in_range)
		...
		if(potential_number < faction_mobs_number)
			potential_faction = faction_to_get
	...
	if(capture_progress == req_capture_progress && faction)
		GLOB.faction_datums[faction].faction_victory_points += home_sector ? 5 : 1
	capture_progress = clamp(capture_progress, 0, req_capture_progress)
	if(!capture_progress && faction)
		get_sector(null)

/obj/structure/prop/sector_center/proc/get_sector(check_faction)
	if(!check_faction)
	...
	if(home_sector)
		if(initial_faction == faction)
			marine_announcement("[name] new controlled by [check_faction], home sector occupied, now for us that big problem!", FACTION_TASKS_SECTOR_CONTROL, null, faction)
			marine_announcement("[name] new controlled by [check_faction], home sector returned back, now your enemy don't can do anything!", FACTION_TASKS_SECTOR_CONTROL, null, initial_faction)
			GLOB.faction_datums[initial_faction].latejoin_enabled = FALSE
		else if(initial_faction == check_faction)
			marine_announcement("[name] new controlled by [check_faction], home sector occupied, enemy now can comeback!", FACTION_TASKS_SECTOR_CONTROL, null, faction)
			marine_announcement("[name] new controlled by [check_faction], home sector returned back, rise and destory our enemies!", FACTION_TASKS_SECTOR_CONTROL, null, initial_faction)
			GLOB.faction_datums[initial_faction].latejoin_enabled = TRUE
		else
...
/obj/structure/prop/sector_center/proc/get_faction_tasks(check_faction)
	var/list/tasks_list = list()
	var/list/potential_tasks = connected_task[check_faction]
	for(var/datum/faction_task/task in potential_tasks)
		if(task.faction == check_faction)
			tasks_list += task
	return tasks_list
...
/datum/faction_task/sector_control/announce_faction()
	marine_announcement("[sector_center] [state & OBJECTIVE_COMPLETE ? announce_desc["complete"] : announce_desc["failed"]]", name, null, faction_owner)
