...
/mob/proc/trainteleport(atom/destination, z_move_flags)
	...
	if(!pulling)
		return zMove(target = destination, z_move_flags = z_move_flags) //No need for a special proc if there's nothing being pulled.
	...
	for(var/atom/movable/AM in conga_line)
		AM.zMove(target = destination, z_move_flags = z_move_flags)
