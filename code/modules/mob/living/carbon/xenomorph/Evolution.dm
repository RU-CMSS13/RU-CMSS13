...
/mob/living/carbon/xenomorph/proc/do_evolve()
	...
	if((!hive.living_xeno_queen) && castepick != XENO_CASTE_QUEEN && !islarva(src) && !hive.allow_no_queen_evo && !Check_Crash())
...
/mob/living/carbon/xenomorph/proc/can_evolve(castepick, potential_queens)
	...
	if(tier == 1 && (((used_tier_2_slots + used_tier_3_slots) / totalXenos) * hive.tier_slot_multiplier) >= 0.5 && castepick != XENO_CASTE_QUEEN)
		...
		return FALSE
	else if(hive.allow_queen_evolve && !hive.living_xeno_queen && potential_queens == 1 && islarva(src) && !Check_Crash() && castepick != XENO_CASTE_DRONE)
