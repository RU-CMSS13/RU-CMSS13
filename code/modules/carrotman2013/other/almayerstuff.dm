...
/datum/authority/branch/evacuation/proc/spawn_sd_effects()
	...
	spawn while(NUKE_EXPLOSION_ACTIVE)
		...
		if(world.time >= dest_start_time + 2600) //3  //Тряска камеры и дымка
		...
		if(world.time >= dest_start_time + 3100) //4  //первый взрыв
			for(var/mob/living/carbon/M in GLOB.mob_list)
				shake_camera(M, 25, 1)
			explosion(pick(GLOB.mainship_pipes), 0, 1, 3, 0)


		if(world.time >= dest_start_time + 3400) //5  //тряска сильнее и взрывы c более сильной
			for(var/mob/living/carbon/M in GLOB.mob_list)
				if(prob(10))
					M.KnockDown(3)
