/obj/item/reagent_container/hypospray/autoinjector/emergency/advanced
	...
	chemname = "emergency"
	amount_per_transfer_from_this = (REAGENTS_OVERDOSE-1)*6 + (MED_REAGENTS_OVERDOSE-1) + (LOWH_REAGENTS_OVERDOSE-1) * 5
	volume = (REAGENTS_OVERDOSE-1)*6 + (MED_REAGENTS_OVERDOSE-1) + (LOWH_REAGENTS_OVERDOSE-1) * 3
	mixed_chem = TRUE
	uses_left = 1
...
/obj/item/reagent_container/hypospray/autoinjector/emergency/advanced/Initialize()
	...
	reagents.add_reagent("arithrazine", LOW_REAGENTS_OVERDOSE-3)
	...
	reagents.add_reagent("iron", REAGENTS_OVERDOSE-1)
	update_icon()
