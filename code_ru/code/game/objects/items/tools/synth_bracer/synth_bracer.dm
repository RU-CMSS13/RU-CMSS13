/obj/item/clothing/gloves/synth
	...
	icon = 'code_ru/icons/obj/items/synth/bracer.dmi'
	icon_state = "bracer_default"
	item_icons = list(
		WEAR_HANDS = 'code_ru/icons/mob/humans/onmob/synth/bracer.dmi'
	)
...
/obj/item/clothing/gloves/synth/proc/has_chip(chip_type)
    for(var/obj/item/device/simi_chip/chip in ability_chips)
        if(istype(chip, chip_type))
            return TRUE
    return FALSE
//#############################
//###### ICON HANDLING ########
...
/obj/item/clothing/gloves/synth/get_mob_overlay(mob/user_mob, slot, default_bodytype = "Default")
	...
	var/image/bracer_img = overlay_image('code_ru/icons/mob/humans/onmob/synth/bracer.dmi', get_icon_state(user_mob, slot), null, RESET_COLOR|NO_CLIENT_COLOR)
