...
/datum/ai_action/converse/get_weight(datum/human_ai_brain/brain)
	...
	if(brain.in_combat || brain.in_conversation || (brain.tied_human.health < brain.tied_human.health_threshold_crit))
		return 0
...
/datum/ai_action/converse/trigger_action()
	...
	for(var/mob/living/carbon/human/nearby_human in view(2, brain.tied_human))
		...
		if(!other_brain || other_brain.in_combat || other_brain.in_conversation || other_brain.tied_human.client || (other_brain.tied_human.health < other_brain.tied_human.health_threshold_crit))
			continue
